---
title: MAS / Agentic AI / RAG / Reasoning — Knowledge Base (Obsidian)
type: index
status: active
created: 2025-12-13
---
...
## Как пользоваться этим хранилищем (чтобы не утонуть, когда статей станет в 3–10 раз больше)

### 1) Простая схема навигации
- **Сначала решай “куда относится”**: *Агенты / MAS / Протоколы / RAG / Графы / Reasoning / Benchmarks / Safety / Архитектуры / UI*  
- **Потом решай “что это”**: *paper / blog / survey / benchmark / framework / whitepaper*  
- **Потом решай “зачем мне это”**: *архитектура / алгоритм / практика / проблемы / оценка / риски*

### 2) Правила добавления нового материала (интуитивно и без мучений)
Когда добавляешь новую ссылку, вставляй её в один из разделов ниже и придерживайся мини-шаблона:
- **One-liner**: зачем это важно (1 строка)
- **Status**: `todo / skimmed / read / applied`
- **Outcome**: что вынести в систему (1–3 буллета)
- **Tags**: оставляй оригинальные, но при желании добавляй 1–3 “системных” тега (например: `#mas`, `#rag`, `#mcp`, `#reasoning`, `#benchmarks`, `#safety`)

> Важно: ниже я **не удалял** и **не сокращал** твой текст. Я только разложил по темам и добавил “скелет” навигации.

---

## Contents
- [[#1 Must have]]
- [[#2 Agentic AI & MAS Architectures]]
- [[#3 RAG, Retrieval, Tooling]]
- [[#4 Graphs, Knowledge Graphs, GraphRAG]]
- [[#5 Reasoning, Prompting, Meta-thinking, Verification]]
- [[#6 Benchmarks & Evaluation]]
- [[#7 Protocols & MCP]]
- [[#8 LLM Architectures & Model Families]]
- [[#9 Agentic Web]]
- [[#10 MAS cooperation & Coordination]]
- [[#11 Reliability, Hallucinations, Safety]]
- [[#12 Interfaces & Human-AI Interaction]]
...
## 1.1 Geometric / Parametric Memory
...
## 1.2 Agentic AI Survey / Frameworks
...
## 1.3 In-Context RL / World Models via Communication
...
## 1.4 Meta-thinking via Multi-agent RL
...
## 1.5 Single-threaded Agent vs Multi-agent (Context Engineering)
...
## 1.6 Production multi-agent orchestration (Anthropic)
...
## 1.7 Dynamic planning + Actor Factory + Progress mgmt
...
## 2.1 Anthropic — important articles
https://www.anthropic.com/engineering/built-multi-agent-research-system  
...
## 2.2 HiVA — evolving hierarchy (Semantic-Topological Evolution)
...
## 3.2 Context pruning / rerank+prune
...
## 3.3 RAG for tool selection (MCP toolsets)
...
## 4.1 Graph-based AI agent (biomed) — KG verify loop
...
## 4.2 GraphRAG for global queries / query-focused summarization
...
## 4.3 KG + RAG in customer support (LinkedIn tickets)
...
## 5.1 Graph problems as general reasoning substrate (GraphPile / GraphMind)
...
- Ablation studies confirm the critical value of ToE and ToE and CoT in building step-by-step, interpretable reasoning.  
...
## 5.2 Prompt auto-optimization + causal graphs (EGO-Prompt)
...
## 5.3 DAG view of math reasoning (DAG-Math)
...
## 5.4 Verification-first prompting
...
## 6.1 MCP agents stress test benchmark
...
## 6.2 Long-context reasoning benchmark (NeedleChain)
...
## 10.1 Role-based RL for MAS (MaGRPO)
# MAS cooperation
...
## 10.2 Galaxy (Cognition Forest, KoRa, Kernel)
<details>
...
## 10.3 Million-step task with zero errors (MDAP / MAKER)
...
## 10.4 Collaboration Gap (coordination failure + relay inference)
# Issues
...
## 11.1 Psychological risk simulation in Human-AI
<details>
...
## 11.2 Hallucination neurons (H-Neurons)
# Hallucinations
...
## 11.3 Grounded medical reasoning (3DReasonKnee)
...
## 12.1 Generative Interfaces (GenUI)
# Others
...
## 12.2 Dialogue world models + belief modeling (DreamCUB)
...
## 13.1 LatentMAS (KV-cache sharing, latent reasoning)
# Hidden state
...
## Agent0 (self-evolving, zero-data, tool-integrated RL)
